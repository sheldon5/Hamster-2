                    ...
                    <form id="form" role="form"
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="title" id="title"
                                   value="${fn:escapeXml(article.title)}">
                            ...
                            <select class="form-control" id="cid" name="cid">
                                <c:forEach items="${categories}" var="cate">
                                    <c:if test="${!empty(cidDefault)}">
                                        <option value="${cate.cid}" ${article.cid==cate.cid?'selected':''}>${fn:escapeXml(cate.cname)}
                                        </option>
                                    </c:if>
                                    <c:if test="${empty(cidDefault)}">
                                        <option id="cid-default" value="${cate.cid}" ${article.cid==cate.cid?'selected':''}>${fn:escapeXml(cate.cname)}
                                        </option>
                        ...
                        <div class="form-group">
                            <label for="deploy">是否发布：</label>
                            <select id="deploy" class="form-control" name="deploy">
                                <option value="0" ${article.deploy==0?'selected':''}>发布</option>
                                <option value="1" ${article.deploy==1?'selected':''}>保存草稿</option>
                            </select>
                            <span>${errors.deploy}</span>
                        </div>
                        <c:if test="${method=='update'}">
                            <div class="form-group">
                                <label for="meta">摘要：</label>
                                <textarea class="form-control" name="meta" id="meta" cols="80"
                                          rows="8">${fn:escapeXml(article.meta)}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            &nbsp;&nbsp;
                            <c:if test="${article.editor==1}">
                                <button class="btn btn-primary" id="btn-editor"
                                        onclick="return editorChange('${pageContext.request.contextPath}/manage/article/editor/','0')">
                                    MarkDown编辑器
                                </button>
                            </c:if>
                            <c:if test="${article.editor==0}">
                                <button class="btn btn-primary" id="btn-editor"
                                        onclick="return editorChange('${pageContext.request.contextPath}/manage/article/editor/','1')">
                                    CKEditor编辑器
                                </button>
                            </c:if>
                            &nbsp;&nbsp;
                            ...
                            &nbsp;&nbsp;
                            <button onclick="return resetFromDatabase('${pageContext.request.contextPath}/manage/article/reset','${pageContext.request.contextPath}')" class="btn btn-primary">
                                reset from db
                            </button>
                            &nbsp;&nbsp;
                            ...
                            <c:if test="${article.editor==1}">
                                <div id="ckeditor">
                                    <textarea class="form-control" name="content"
                                              id="content">${fn:escapeXml(article.content)}</textarea>
                                </div>
                            </c:if>
                            <c:if test="${article.editor==0}">
                                <div id="markdown">
                                <textarea class="form-control" style="display:none;"
                                          name="md" id="md">${fn:escapeXml(article.md)}</textarea>
                                </div>
                            </c:if>
                            </div>
                        </div>
                        <div class="text-right">
                            <button class="btn btn-primary" onclick="return submitArticle()">
                                提交
                            </button>
                        </div>
                    </form>
